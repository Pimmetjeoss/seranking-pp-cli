...
type Config struct {
	...
	Path                 string            `toml:"-"`
	SeRankingApiKeyQuery string            `toml:"ranking_api_key_query"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("SERANKING_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "seranking-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if cfg.AuthSource == "" && cfg.SeRankingApiKeyQuery != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("SERANKING_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.SeRankingApiKeyQuery
	if token == "" {
		return ""
	}
	if c.SeRankingApiKeyQuery == "" {
		return ""
	}
	return token
}

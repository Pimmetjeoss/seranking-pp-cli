...
import (
	...
	"seranking-pp-cli/internal/client"
	"seranking-pp-cli/internal/config"
...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  seranking-pp-cli doctor
  seranking-pp-cli doctor --json
  seranking-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					// PATCH: prefer the human-friendly alias in diagnostics while retaining
					// SE_RANKING_API_KEY_QUERY compatibility in config.Load.
					report["auth_hint"] = "export SE_RANKING_API_KEY=<your-key>"
				} else {
					authConfigured = true
			...
			// PATCH: Treat SE_RANKING_API_KEY as a first-class equivalent of the
			// generated SE_RANKING_API_KEY_QUERY variable for doctor output.
			if os.Getenv("SE_RANKING_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "SE_RANKING_API_KEY")
			} else if os.Getenv("SE_RANKING_API_KEY_QUERY") != "" {
				authEnvSet = append(authEnvSet, "SE_RANKING_API_KEY_QUERY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SE_RANKING_API_KEY")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'seranking-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'seranking-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'seranking-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'seranking-pp-cli sync' to refresh."
	}
	return report
}
